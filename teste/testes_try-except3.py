...
def listar_produtos():
    ...
    for id, info in produtos.items():
        print(f"{id}: {info['nome']} - R${info['preco']:.2f} | Estoque: {info['Quantidade']}")
...
def adicionar_ao_carrinho():
    ...
    try:
        ...
        if id in produtos:
            estoque_disponivel = produtos[id]["Quantidade"]
            quantidade = int(input(f"Quantidade (em estoque: {estoque_disponivel}): "))
            ...
            if quantidade > estoque_disponivel:
                print(f"❌ Quantidade solicitada maior que o estoque disponível ({estoque_disponivel}).")
                return

            if id in carrinho:
                if carrinho[id]["quantidade"] + quantidade > estoque_disponivel:
                    print("❌ Soma total no carrinho excede o estoque disponível.")
                    return
...
def finalizar_compra():
    ...
    # Atualizar estoque
            for id, item in carrinho.items():
                produtos[id]["Quantidade"] -= item["quantidade"]
